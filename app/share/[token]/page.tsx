...
export default function SharePage() {
  ...
  const [share, setShare] = useState<ShareMeta>({ name: "Shared Drive", canDownload: true, expiresAt: null });
  ...
  function downloadSelected() {
    const selectedFiles = items.filter((item) => selectedPaths.has(item.path) && item.type !== "folder" && item.directDownloadUrl);
  ...
  return (
    <main className="min-h-screen bg-[#08090d] text-zinc-100">
      <header className="sticky top-0 z-40 border-b border-white/10 bg-[#08090d]/95 px-3 py-3 backdrop-blur md:px-4">
        <div className="mx-auto flex max-w-7xl flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
            <div className="flex items-center gap-2 rounded-lg border border-white/10 bg-white/[0.04] px-3 py-2 md:w-80">
      ...
      <div className="mx-auto max-w-7xl p-3 md:p-4">
        {notice ? <div className="mb-3 rounded-lg border border-white/10 bg-white/[0.04] px-3 py-2 text-sm text-zinc-200">{notice}</div> : null}
        {error ? (
          <div className="flex min-h-[70vh] flex-col items-center justify-center gap-3 text-center">
            ...
            <h2 className="text-xl font-semibold text-white">Share link unavailable</h2>
          ...
          <div className="space-y-3">
                ...
                <nav className="flex flex-wrap items-center gap-1 text-sm">
                  ...
                  {breadcrumbs.map((crumb, index) => (
                    <div key={`${crumb}-${index}`} className="flex items-center gap-1">
                      ...
                      <button onClick={() => void loadFolder(breadcrumbs.slice(0, index + 1).join("/"))} className="max-w-[160px] truncate rounded-lg px-2 py-2 text-zinc-300 hover:bg-white/10">{crumb}</button>
                    </div>
                  ))}
                </nav>
                <div className="flex flex-wrap items-center gap-2 text-xs text-zinc-500">
                  <span><b className="text-white">{filtered.length}</b> items</span>
                  <span><b className="text-white">{formatBytes(displayedBytes)}</b> displayed</span>
                  <span className="rounded-md border border-white/10 bg-white/[0.04] px-2 py-1">{share.canDownload ? "View + Download" : "View only"}</span>
                </div>
              </div>
            ...
            {selectedPaths.size > 0 ? (
              <div className="sticky bottom-3 z-30 flex flex-wrap items-center gap-2 rounded-lg border border-white/10 bg-[#101217]/95 p-2 shadow-2xl backdrop-blur">
                <span className="rounded-lg border border-white/10 px-3 py-2 text-sm text-zinc-200">{selectedPaths.size} selected</span>
                {share.canDownload ? <button onClick={downloadSelected} className="inline-flex items-center gap-2 rounded-lg bg-[#d7ff3f] px-3 py-2 text-sm font-semibold text-black"><Download className="h-4 w-4" />Download selected</button> : null}
                <button onClick={() => setSelectedPaths(new Set())} className="inline-flex items-center gap-2 rounded-lg border border-white/10 px-3 py-2 text-sm text-zinc-200"><X className="h-4 w-4" />Clear</button>
              </div>
            ) : null}
...
function ShareCard({
  ...
  return (
    <div className="flex min-h-[230px] flex-col rounded-lg border border-white/10 bg-black/20 p-3 transition hover:border-white/20 hover:bg-white/[0.05]">
      <div className="mb-3 flex items-center justify-between">
        ...
        {item.type === "folder" && canDownload ? (
          <a
            ...
            onClick={(event) => {
              if (!window.confirm("Folder ZIP can be slow for huge folders. For very large assets, download files individually.")) event.preventDefault();
            }}
          ...
          <a href={item.directDownloadUrl} className="rounded-lg bg-[#d7ff3f] p-2 text-black" aria-label={`Download ${item.name}`}>
...
function ShareRow({
  ...
  return (
    <div className={`grid gap-2 px-3 ${compact ? "py-1.5" : "py-2"} md:grid-cols-[42px_minmax(0,1fr)_100px_120px_74px_56px] md:items-center`}>
      <SelectionCheckbox checked={checked} onClick={() => onToggleSelect(item.path)} />
      <button onClick={() => onOpen(item)} className="grid min-w-0 grid-cols-[40px_minmax(0,1fr)] items-center gap-3 text-left">
        ...
        <span className="min-w-0">
          ...
          <span className="mt-1 block truncate text-xs text-zinc-500">{typeLabel(item)} - {previewStatusLabel(item)}</span>
        </span>
      </button>
