...
async function updatePreviewMetadataFile(
  fileName: "preview-queue.json" | "preview-jobs.json",
  oldPath: string,
  newPath: string
) {
...
async function updateMoveRelatedMetadata(moves: Array<{ oldPath: string; newPath: string }>) {
  ...
  for (const move of moves) {
    ...
    summary.updatedPreviewQueue += await updatePreviewMetadataFile(
      "preview-queue.json",
      move.oldPath,
      move.newPath
    );
    summary.updatedPreviewJobs += await updatePreviewMetadataFile(
      "preview-jobs.json",
      move.oldPath,
      move.newPath
    );
...
function buildMovePlan(paths: string[], targetFolder: string) {
